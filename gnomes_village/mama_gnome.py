...
def router(model, tokenizer, user_input: str) -> str:
    ...
    JSON schema (output this exact structure, with no surrounding fences or text):
    {
      ...
      "task_think": "reasoning sub-task for the thinking gnome, or null",
      "task_direct": "direct sub-task for the fast gnome, or null",
...
def synthesize(model, tokenizer, thinking_gnome, direct_gnome, initial_plan, user_question: str) -> str:
    sys_prompt = """
    You are Mama Gnome — the wisest gnome in the village and a master planner.
    You dispatched two small gnomes to help answer a traveler's question. They have returned with their responses.
    Your job now is to synthesize their work into a single, clear, final answer for the traveler.
    Read both gnome responses carefully. Each gnome ends their response with a confidence score (1-10) — use it to weigh their answers: higher confidence means more reliable input. Resolve any gaps or disagreements and produce the best possible final answer.
    """
